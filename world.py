...
class World():
    def __init__(self):
        ...
        self.seed = random.randint(-9999, 9999)
        self.chunks = {}
        self.load_chunks = {}
        self.load_distance = 3
        self.scroll = [-160, -160]
        ...
        for x in range(self.load_distance * 2 + 1):
            for y in range(self.load_distance * 2 + 1):
                self.chunks[f"{x - 3}_{y - 3}"] = chunk.Chunk((x - 3, y - 3), self.seed)
            ...
            for x in range(self.load_distance * 2 + 1):
                for y in range(self.load_distance * 2 + 1):
                    ...
                    try:
                        ...
                        self.chunks[load_chunk_pos] = chunk.Chunk((self.center_chunk[0] + x - 3, self.center_chunk[1] + y - 3), self.seed)
                        self.load_chunks[load_chunk_pos] = self.chunks[load_chunk_pos]
        self.last_center_chunk = self.center_chunk.copy()

    def set_and_remove_blocks(self):#установка и разрушение блоков
        mousepos = pygame.mouse.get_pos()#позиция указателя мыши
        ...
        if pygame.mouse.get_pressed()[2]:#сломать блок
            if clicked_block.name != "air":
                clicked_chunk.blocks[blockpos[0]][blockpos[1]] = blocks.Air(blockpos)
        elif pygame.mouse.get_pressed()[0]:#установить блок
            if clicked_block.name == "air":
                clicked_chunk.blocks[blockpos[0]][blockpos[1]] = blocks.Stone(blockpos)
